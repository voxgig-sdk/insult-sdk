// Command Insult-cli is a CLI + REPL for the Insult SDK,
...
const entitiesHelp = "adjective adjectiveformat insult insultformat"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      INSULT_APIKEY (api key), INSULT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: INSULT_APIKEY carries the API key and
	// INSULT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("INSULT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("INSULT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewInsultSDK(opts)
