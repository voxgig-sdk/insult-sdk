...
boru-driven command-line client **and** interactive REPL for the Insult
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/insult-cli)
make build
...
./insult-cli load 1 adjective            # {id:1} shorthand
./insult-cli load '{id:1}' adjective       # explicit match map

# 5. Override the API base URL for a single call
INSULT_BASE=https://api.example.com ./insult-cli load 1 adjective

# 6. No arguments -> interactive REPL
./insult-cli
insult> load 1 adjective
insult> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/insult-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/insult-cli load 1 adjective
   ```
...
```sh
./insult-cli load 1 adjective          # scalar shorthand for {id:1}
./insult-cli load '{id:1}' adjective     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `insult>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `adjective`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/insult-cli`. |
...
(`./insult-cli <expr>`) and interactively (the REPL), and expressions compose the
