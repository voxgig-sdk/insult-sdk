...
{
    private static function utility(): InsultUtility
    {
        return new InsultUtility();
    }
    ...
    private static function ctx(array $over = []): InsultContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new InsultContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new InsultOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof InsultError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new InsultOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = InsultMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new InsultOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = InsultMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new InsultOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = InsultMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new InsultSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = InsultMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new InsultResponse([]),
            'result' => new InsultResult([]),
        ]);
        [, $err] = InsultMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new InsultSpec(['step' => 's']),
            'response' => new InsultResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new InsultResult([]),
        ]);
        [, $err] = InsultMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new InsultSpec(['step' => 's']),
            'response' => new InsultResponse(self::resp(200, ['v' => 1])),
            'result' => new InsultResult([]),
        ]);
        [, $err] = InsultMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new InsultSpec(['step' => 's']),
            'response' => new InsultResponse(self::resp(200, ['v' => 2])),
            'result' => new InsultResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        InsultMakeResponse::call($ctx);
    ...
    {
        $preset = new InsultResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new InsultSpec([]),
            'response' => new InsultResponse([]),
            'result' => new InsultResult([]),
        ...
        [$got, $err] = InsultMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new InsultResult([])]);
        [, $err] = InsultMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new InsultSpec([]), 'result' => null]);
        [, $err] = InsultMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new InsultSpec(['step' => 's']),
            'result' => new InsultResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new InsultOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = InsultMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new InsultSpec(['step' => 's']),
            'result' => new InsultResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new InsultOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = InsultMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new InsultResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new InsultSpec([]),
            'result' => new InsultResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = InsultMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = InsultMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new InsultError('boom', 'boom');
        $utility->fetcher = function (InsultContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new InsultSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = InsultMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (InsultContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = InsultMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(InsultResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (InsultContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new InsultSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = InsultMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new InsultResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new InsultSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = InsultMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = InsultMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = InsultMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new InsultResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, InsultDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new InsultResult(['ok' => false])]);
        $this->expectException(InsultError::class);
        InsultDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new InsultResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', InsultMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new InsultBaseFeature();
        $b = new InsultBaseFeature();
        InsultFeatureAdd::call($ctx, $a);
        InsultFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): InsultBaseFeature
    {
        $f = new InsultBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        InsultFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): InsultContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new InsultSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = InsultPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new InsultResponse(['headers' => null]),
            'result' => new InsultResult([]),
        ]);
        InsultResultHeaders::call($ctx);
