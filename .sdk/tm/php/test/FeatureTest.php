...
class FtCtrl extends InsultControl
{
...
class FtClient
{
    ...
    private InsultUtility $utility;

    public function __construct(InsultUtility $utility, array $options)
    {
    ...
    public function get_utility(): InsultUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (InsultContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (InsultContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = InsultConfig::make_feature($name);
    ...
    private static function buildUrl(InsultSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof InsultError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new InsultResponse($fetched);
        }
    ...
    private function fail(InsultContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (InsultContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
