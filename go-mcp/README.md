...
[MCP](https://modelcontextprotocol.io) server exposing the Insult SDK as
two agent tools — `insult_list` and `insult_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/insult-mcp)
make build

# 2. Provide credentials via the environment
export INSULT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user insult \
  -- /absolute/path/to/insult-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./insult-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// insult_list: first page of records
{ "entity": "adjective" }
{ "entity": "adjective", "query": { } }

// insult_load: one record by id
{ "entity": "adjective", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/insult-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user insult \
     -- "$PWD"/dist/*/insult-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `insult_list` and `insult_load` tools now appear
   in new sessions. Ask the agent to *"list adjective using insult"*
   and it calls `insult_list` with `{"entity":"adjective"}`.
...
```sh
export INSULT_APIKEY=sk_live_xxx            # API key
export INSULT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `insult_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `insult_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `INSULT_APIKEY` | API key sent with every request. |
| `INSULT_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

adjective | adjectiveformat | insult | insultformat

### Smoke test via HTTP (raw JSON-RPC)

```sh
./insult-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"insult_load","arguments":{"entity":"adjective","query":{"id":1}}}}'
```
