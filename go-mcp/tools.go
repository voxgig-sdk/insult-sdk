...
type Args struct {
	Entity string         `json:"entity" jsonschema:"adjective | adjectiveformat | insult | insultformat"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.InsultSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "insult_list",
		Description: "List records from Insult. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "insult_load",
		Description: "Load a single record from Insult. " +
...
func runOp(client *sdk.InsultSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.InsultSDK, name string) (sdk.InsultEntity, error) {
	switch strings.ToLower(name) {
	case "adjective":
		return client.Adjective(nil), nil
	case "adjectiveformat":
		return client.Adjectiveformat(nil), nil
	case "insult":
		return client.Insult(nil), nil
	case "insultformat":
		return client.Insultformat(nil), nil
