...
local InsultEntity = {}
InsultEntity.__index = InsultEntity


function InsultEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InsultEntity)
  self._name = "insult"
  self._client = client
...
function InsultEntity:get_name()
  return self._name
end


function InsultEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InsultEntity.new(self._client, opts)
end
...
function InsultEntity:mark_deleted()
  self._deleted = true
end


function InsultEntity:deleted()
  return true == self._deleted
end


function InsultEntity:data_set(args)
  if args ~= nil then
...
function InsultEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InsultEntity:match_set(args)
  if args ~= nil then
...
function InsultEntity:match_get()
...
function InsultEntity:stream(action, args, callopts)
...
function InsultEntity:_run_op(ctx, post_done)
  local utility = self._utility
