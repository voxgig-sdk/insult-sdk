...
The Lua SDK for the Insult API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Adjective(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Insult(nil):load(
  { id = "test01" }, nil
)
