...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        InsultSDK $client,
        InsultUtility $utility
    ): InsultContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new InsultContext($ctxmap, null);
    ...
    private static function fixctx(InsultContext $ctx, InsultSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?InsultError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new InsultError($code, $msg);
    }
    ...
    {
        $hook_client = InsultSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends InsultBaseFeature {
            public $hook_fn = null;
            public function TestHook(InsultContext $ctx): void
            {
    ...
    {
        $init_client = InsultSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends InsultBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new InsultSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = InsultSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = InsultSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof InsultError) ? $result->err->msg : (string)$result->err;
