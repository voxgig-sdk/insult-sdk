...
describe('InsultEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'insult.' + op, live)) return
    }
    ...
    let insult_ref01_data = Object.values(setup.data.existing.insult)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/insult/InsultTestData.json')
  ...
  const idmapEnvVal = process.env['INSULT_TEST_INSULT_ENTID']
  ...
  idmap = env['INSULT_TEST_INSULT_ENTID']
