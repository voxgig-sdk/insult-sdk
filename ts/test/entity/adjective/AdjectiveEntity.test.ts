...
describe('AdjectiveEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'adjective.' + op, live)) return
    }
    ...
    let adjective_ref01_data = Object.values(setup.data.existing.adjective)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/adjective/AdjectiveTestData.json')
  ...
  const idmapEnvVal = process.env['INSULT_TEST_ADJECTIVE_ENTID']
  ...
  idmap = env['INSULT_TEST_ADJECTIVE_ENTID']
