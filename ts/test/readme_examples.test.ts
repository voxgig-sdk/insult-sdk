...
//     (\`new InsultSDK(...)\` / \`InsultSDK.test(...)\` become
//     \`InsultSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"adjective":{"test01":{"id":"test01"}},"adjectiveformat":{"test01":{"id":"test01"}},"insult":{"test01":{"id":"test01"}},"insultformat":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new InsultSDK(...)\` and any
// \`InsultSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+InsultSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/InsultSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
