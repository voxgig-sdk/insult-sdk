...
class InsultTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            InsultTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = InsultTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = InsultTestRunner.getenv("INSULT_TEST_LIVE")
        override = InsultTestRunner.getenv("INSULT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = InsultTestRunner.getenv(key)
        ...
        explain = InsultTestRunner.getenv("INSULT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["INSULT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if InsultTestRunner._test_control is not None:
            return InsultTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                InsultTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            InsultTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = InsultTestRunner.load_test_control()
...
def load_env_local():
    InsultTestRunner.load_env_local()


def env_override(m):
    return InsultTestRunner.env_override(m)


def entity_data(v):
    return InsultTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return InsultTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return InsultTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return InsultTestRunner.load_test_control()


def live_delay_ms():
    return InsultTestRunner.live_delay_ms()
